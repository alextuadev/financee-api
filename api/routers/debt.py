...
async def create_debt(request: Request, debt: Debt):
    ...
    return JSONResponse(status_code=201, content=new_debt)
...
async def get_debts(request: Request):
    ...
    return JSONResponse(status_code=200, content=debts)
...
async def get_debt(request: Request, id: int = Path(ge=1, le=2000)):
    ...
    return JSONResponse(status_code=200, content=debt)
...
async def update_debt(request: Request, debt: Debt, id: int = Path(ge=1, le=2000)):
    ...
    async with async_session() as db:
        ...
        if not updated_debt:
            raise HTTPException(status_code=404, detail="Debt not found")
    return JSONResponse(status_code=200, content=updated_debt)
